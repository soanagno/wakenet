...
def Superposition(inpt1,
    ...
    for p in range(xs.size):
        ...
        for ii in range(int(hub_tot/2)):
            ...
            mean_hub_speed = np.mean([domain[hub_start+ii], domain[hub_finish-ii]])
            ...
            # Calculate inlet speed cubed (kinetic energy) term that will go in the calculation of power.
            inlet_speed_power += mean_hub_speed * mean_hub_speed * mean_hub_speed * 2 * np.pi * int((hub_tot/2-ii)/dy) * 1/dy
    ...
    if plots == True or timings == True:
        ...
        # ----- FLORIS wake plots ----- #
        if contourss == True:
            X, Y = np.meshgrid(np.linspace(0, new_len, u_mesh.shape[1]), np.linspace(new_height1, new_height2, u_mesh.shape[0]))
            contours = axs[0].contour(X, Y, u_mesh, 1, colors='white')
            axs[0].clabel(contours, inline=True, fontsize=8)

        im1 = axs[0].imshow(u_mesh, vmin=vmin, vmax=vmax, cmap=cmap, extent=[0, new_len, new_height1, new_height2])
        ...
        # ----- DNN wake plots ----- #
        if contourss == True:
            X, Y = np.meshgrid(np.linspace(0, new_len, domain_final.shape[1]), np.linspace(new_height2, new_height1, domain_final.shape[0]))
            contours = axs[1].contour(X, Y, domain_final, 1, colors='white')
            axs[1].clabel(contours, inline=True, fontsize=8)

        im2 = axs[1].imshow(domain_final, vmin=vmin, vmax=vmax, cmap=cmap, extent=[0, new_len, new_height1, new_height2])
        ...
        im3 = axs[2].imshow((np.abs(u_mesh - domain_final)/max_val*100), cmap=cmap, extent=[0, new_len, new_height1, new_height2], vmax=20)
        ...
        step = int(u_mesh.shape[1]/(transects+2))  # defines the step between the downstream transects
        for indx in range(step, u_mesh.shape[1]-2*step+1, step):

            yy1 = u_mesh[:, indx]  # FLORIS transect
            yy2 = domain_final[:, indx]  # CNN transect

            axs[cnt].plot(np.flip(yy1, axis=0), np.arange(u_mesh.shape[0]), color='navy', linestyle='--')
            axs[cnt].plot(np.flip(yy2, axis=0), np.arange(u_mesh.shape[0]), color='crimson')
    ...
    if optimize == True:
        ...
        cp_interp = np.cos(np.pi/180*(-yws))**(1.0)*cp_interp
        # Calculate powers using the kinetic energy term
        power_tot = rho*0.5*cp_interp*hub_speeds_power**3*np.pi*D*D/4
