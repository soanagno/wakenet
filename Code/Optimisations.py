...
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa55555
def florisOptimiser(ws,
         ti,
         layout_x,
         layout_y,
         min_yaw=-30,
    ...
    file_dir = os.path.dirname(os.path.abspath(__file__))
        ...
        os.path.join(file_dir, file_path)
    )
    ...
    if mode == 'yaw':
        ...
        boundaries = [[0, 0], [0, opt_ybound*D], [opt_xbound*D, opt_ybound*D], [opt_xbound*D, 0]]
...
def neuralOptimiser(ws,
    ...
    if mode == 'yaw':
        ...
        power_ini = -Superposition(np.zeros(xs.size), layout, u_stream=ws, tis=ti, cp=cp, wind_speed=wind_speed, plots=plots_ini, optimize=True)
        ...
        # Optimise and time
        t0 = time.time()
        res = minimize(Superposition, x0, args=(layout, ws, ti, cp, wind_speed), method='SLSQP', bounds=bnds, options = opt_options)
        t1 = time.time()
        ...
        # optimal, floris_power_gain = Superposition(res.x, u_stream=ws, tis=ti, xs=xs, ys=ys, cp=cp, wind_speed=wind_speed, plots=plots, optimize=True, floris_gain=floris_gain)
        optimal, floris_power_opt = Superposition(res.x, layout, u_stream=ws, tis=ti, cp=cp, wind_speed=wind_speed, plots=plots, optimize=True, floris_gain=floris_gain)
        ...
        if plots == True:
            ...
            print('Neural Power Gain (%)', round((np.abs(optimal)-power_ini)/power_ini*100, 2))
            print('|-----------------------|')
        ...
        # Calculate initial power
        power_ini = -Superposition(x0, np.zeros(xs.size), u_stream=ws, tis=ti, cp=cp, wind_speed=wind_speed, farm_opt=farm_opt, plots=plots_ini, optimize=True)
        ...
        # Optimise and time
        t0 = time.time()
        res = minimize(Superposition, layout, args=(yws, ws, ti, cp, wind_speed, farm_opt), method='SLSQP', bounds=bnds, options = opt_options, constraints=tmp1)
        t1 = time.time()
        ...
        # optimal, floris_power_gain = Superposition(res.x, u_stream=ws, tis=ti, xs=xs, ys=ys, cp=cp, wind_speed=wind_speed, plots=plots, optimize=True, floris_gain=floris_gain)
        optimal, floris_power_opt = Superposition(res.x, yws, u_stream=ws, tis=ti, cp=cp, wind_speed=wind_speed, farm_opt=farm_opt, plots=plots, optimize=True, floris_gain=floris_gain, x0=x0)
...
def Compare(yws, ws, ti, xs, ys, print_times=True, timings=False, optimize=True):
    """
    ...
    wind_speed = np.array(data['turbine']['properties']['power_thrust_table']['wind_speed'])

    # power_ini = Superposition([-0, 0, -0], u_stream = ws, tis=ti, cp=cp, wind_speed=wind_speed, plots=False, optimize=True)
    # print('Initial Power', round(power_ini/1e6, 2), 'MW')

    return Superposition(yws, layout, u_stream=ws, tis=ti, cp=cp, wind_speed=wind_speed, plots=True, optimize=optimize, print_times=print_times, timings=timings, floris_gain=True)


def Assess(xs, ys, res=10, farm_opt=False):
    """
    ...
    t1 = np.zeros((res, res)); t2 = np.zeros((res, res))
    ...
    for k1 in range(res):
        ...
        for k2 in range(res):
            ...
            if farm_opt == True:
                ...
                g1[k1, k2], t1[k1, k2], g0[k1, k2] = florisOptimiser(ws=x_ws[k1], ti=y_ti[k2], layout_x=xs, layout_y=ys)
                g2[k1, k2], t2[k1, k2] = neuralOptimiser(ws=x_ws[k1], ti=y_ti[k2], xs=xs, ys=ys, floris_gain=True)
...
def MakeHeatmap(bitmap, x_ws, y_ti, vmax=None, title=None):
    ...
    x_min = np.min(x_ws); x_max = np.max(x_ws)
    y_min = np.min(y_ti); y_max = np.max(y_ti)
    ...
    plt.imshow(bitmap, cmap='RdYlGn', interpolation='nearest',
                        vmin=-maxval, vmax=maxval, extent = [x_min , x_max, y_min , y_max], aspect = (x_max-x_min)/(y_max-y_min))
